"""Three figures for the exhaustive ensemble-combination search
(build_all_combinations.py). Reads combos_summary.csv,
...
combo = slot 1) -- same bar/line adjacent-pair context already validated
for this exact order/palette in
experiments/RQ1/experiment_2/build_ablation_comparison_figures.py.
Persistence is a reference, not a competing series: muted gray, dashed,
no marker -- distinct from the categorical set on purpose.
...
def figure1_overall_bar(summary, top5_labels, busker):
    ...
    # display top-ranked at the top of the chart
    ...
    ax.set_title("Figure 1 -- Overall performance of the top 5 ensemble combinations\n"
                  "(exhaustive search over all 26 size-2..5 subsets of 5 RQ2 architectures)")
...
def figure2_by_lead_allzones(detail, persistence, busker, top5_labels):
    ...
    ax.set_xticks(LEADS)
    ax.set_title("Figure 2 -- Top 5 ensemble combinations by lead time\n(all zones, all livelihood regions pooled)")
...
def figure3_by_lead_per_region(detail, persistence, busker, top5_labels):
    ...
    fig.suptitle("Figure 3 -- Top 5 ensemble combinations by lead time, per livelihood-zone region", fontsize=12)
