"""Extended-test-window (2020-02 to 2024-10) counterpart of
`../all_combinations/build_all_combinations.py` -- same exhaustive
26-combination search, same scoring/ranking convention, but on the
extended window instead of the default 2020-2022 Busker-parity window.

Unlike the default-window search, every member here uses a genuinely
matched recipe (delta target, climate cluster excluded for XGBoost/
RandomForest) rather than mixing recipes:
  - XGBoost/RandomForest: freshly fit for this comparison
    (`unhcr_delta_noclimate_extended`, `rf_delta_noclimate_extended`,
    `--target-mode delta --exclude-feature-cluster climate --test-start
    2020-01 --test-end 2024-12`) -- the extended-window recipe did NOT
    already exist; the only prior extended-window XGBoost/RandomForest
    runs (`rq2_delta_test2020to2024`, `rf_delta_extended`) were the plain
    full-feature-set delta variant, not the no-climate recipe the
    default-window ensemble actually uses. Built fresh here specifically
    so this comparison is apples-to-apples with `all_combinations/`.
  - LSTM (`lstm_delta`, window=="extended"), TabICLv2 (`tabicl_level`,
    window=="extended"): already had a matching extended-window
    prediction file, unchanged.
  - T-GCN (`tgcn_ce`, window=="extended"): its own extended file runs to
    2026-06 (T-GCN's own report never restricted it to 2024), not 2024-10
    like the other four -- trimmed to <=2024-10 here so the key set is
    identical across all 5 members before any averaging happens.

Confirmed before running: all 5 members' (time, zone_code, lead) key sets
are identical post-trim (9,660 rows: 92 zones x 15 assessment months x
7 leads).
"""
...
MEMBERS = {
    "XGB": ("../../../RQ1/experiment_2/unhcr_delta_noclimate_extended/predictions.parquet", None),
    "RF": ("../../experiment_4/rf_delta_noclimate_extended/predictions.parquet", None),
    "LSTM": ("../../experiment_1/lstm_delta/predictions.parquet", "extended"),
    "TabICL": ("../../experiment_2/tabicl_level/predictions.parquet", "extended"),
    "TGCN": ("../../experiment_3/tgcn_ce/predictions.parquet", "extended"),
}
...
def load_member(name, rel_path, window):
...
def load_all_members():
    members = {name: load_member(name, path, window) for name, (path, window) in MEMBERS.items()}
    ref_key = members["XGB"].index
    for name, df in members.items():
        assert set(df.index) == set(ref_key), f"key mismatch: {name} ({len(df)} vs {len(ref_key)} rows)"
...
def main():
    ...
    print(f"=== All {len(combos)} combinations (extended window, 2020-02..2024-10), ranked by mean_f1_all ===")
