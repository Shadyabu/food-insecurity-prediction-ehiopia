"""RQ4 Experiment 1 -- top-k vs bottom-k SHAP feature removal (faithfulness).

H0: removing the top-k SHAP-ranked features does not produce greater
weighted-F1 degradation than removing the bottom-k SHAP-ranked features.

Three subjects (see ../common.py module docstring for the full scope
decisions and their rationale, confirmed with the project owner
2026-08-27): XGBoost alone (unhcr_delta_noclimate), RandomForest alone
(rf_delta_noclimate), and the 4-way ensemble (XGBoost+RandomForest+LSTM+
TabICLv2, this project's best result, F1=0.7218 default window). No model
is retrained anywhere in this script -- only already-fitted models'
predict() is re-run against masked test-set inputs.

k-sweep expressed as a percentage of each subject's own feature-ranking
vocabulary size (10%/25%/50%), not a fixed raw k, since vocabulary size
differs by subject (XGBoost/RandomForest: ~215 engineered features;
Ensemble: union of that engineered vocabulary with LSTM's separate
322-column raw-panel vocabulary).

Scope: pooled across all 7 leads (0,1,2,3,4,8,12) and all 3 livelihood-zone
clusters, on the default test window (828 rows/lead).
"""
