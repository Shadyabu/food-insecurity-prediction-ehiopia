...
OUT_DIR = RQ5_DIR / "shap_cache_2020_2024"

# 2026-09-02 rebuild on the project's revised primary window
# (docs/dissertation_plan.md section 6): "automated half only" per the
# project owner's explicit scope decision -- extends the local SHAP +
# rule-engine mapping to 2020-2024 (more distributional data), but Phase
# 4's accuracy-vs-ground-truth scoring stays on the original 14
# hand-labeled cases (2020-2022 only) since new ground truth for 2023-2024
# would require new manual labeling from real FEWS NET reports, which only
# the project owner can do (and already declined a second labeling pass at
# Phase 3). This file (and its two downstream siblings,
# run_batch_rule_engine_2020_2024.py / aggregate_zone_period_2020_2024.py)
# write to entirely separate output paths -- the original shap_cache/,
# predicted_causes.csv, case_zone_period_aggregates.csv are untouched, so
# validate_phase4.py's existing 14-case result is unaffected.
